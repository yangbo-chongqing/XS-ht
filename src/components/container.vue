    ...
    >
      ...
      <template v-for="(item, index) in data">
        ...
        <div v-if="item.type === 'container'" :key="item.key" style="margin: 2px; height: calc(100% - 5px)">
          <div
            :class="{'dr-area': true, 'dr-active': (localSelector && localSelector.key === item.key)}"
            style="height: 100%"
        ...
        <el-container v-else-if="item.type === 'layout'" :key="item.key">
          <el-row
            :class="{
              'dr-area': true, 'dr-active': (localSelector && localSelector.key === item.key)}"
            style="margin: 2px"
            :gutter="item.gutter"
            type="flex"
            ...
            <div style="overflow: auto">
              <el-form-item :label-width="item.labelWidth" :size="item.size" :label="item.name">
                <el-checkbox-group v-model="local.checkbox">
                  <el-checkbox v-for="o in item.option" :key="o.value" :disabled="o.disabled" :label="o.label" />
                </el-checkbox-group>
              </el-form-item>
            </div>
          </div>
