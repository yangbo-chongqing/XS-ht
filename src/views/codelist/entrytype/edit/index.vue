...
<script>
import { postTypeEdit,postTypeDetail } from "@/api/entrycode";
...
export default {
  ...
  methods: {
    ...
    getType(){
      ...
      postTypeDetail(this.qs.stringify(params)).then((res) => {
        loading.close();
        if (res.status == 200) {
          this.form.name = res.data.info.type_name;
          this.form.sort = res.data.info.sort;
        }
      });
    },
    back() {
