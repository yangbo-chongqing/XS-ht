<template>
  <div class="scancode-container">
    ...
    <div class="scancode-tab">
      <span
        :class="dateTabIndex == 1 ? 'active' : ''"
        @click="toggleDate(1)"
      >昨日</span>
      <span
        :class="dateTabIndex == 2 ? 'active' : ''"
        @click="toggleDate(2)"
      >今日</span>
      <span
        :class="dateTabIndex == 3 ? 'active' : ''"
        @click="toggleDate(3)"
      >七日内</span>
      <span
        :class="dateTabIndex == 4 ? 'active' : ''"
        @click="toggleDate(4)"
      >30日内</span>
      <span
        :class="dateTabIndex == 5 ? 'active' : ''"
        @click="dateTabIndex = 5"
      >自定义</span>
      <el-date-picker
        v-if="dateTabIndex == 5"
        v-model="dateZdy"
    ...
    <div v-if="codeData" class="scancode-data">
      ...
      <div class="scancode-body">
        <el-row v-for="(item,index) in codeData" :key="index">
          <el-col :span="20"><p>{{ item.name }}</p></el-col>
          <el-col
            :span="4"
